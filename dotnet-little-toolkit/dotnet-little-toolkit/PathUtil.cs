...
public class PathUtil {
    ...
    private static readonly Lazy<DirectoryPathComparer> lazyDirectoryPathComparer = new(() => new DirectoryPathComparer());
    private static DirectoryPathComparer directoryPathComparer => lazyDirectoryPathComparer.Value;
    ...
    public static string appendPathString(string orgPath, params string[] appendPaths) {
        ...
        foreach (var ap in appendPaths.Distinct(directoryPathComparer)) {
            ...
            if (!paths.Any(p => isEqualDirectoryName(path, p))) {
